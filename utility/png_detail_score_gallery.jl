...
try
    ...
    Pkg.add("FileIO"); @eval using FileIO
end
try
    @eval using ImageIO
catch
    @warn "ImageIO non trovato: lo aggiungo all'ambiente attivo…"
    Pkg.add("ImageIO"); @eval using ImageIO
end
...
function parse_args(args::Vector{String})
    ...
    dir = args[1]; recursive=false; stride=2; gedge=25; maxshow=200; thumb_w=256; sortkey=:score; outpath="gallery.html"
    for a in args[2:end]
        if a=="--recursive"
            recursive=true
...
function main()
    ...
    @threads for i in eachindex(files)
        f = files[i]
        ed = NaN; lv = NaN; lvlog = NaN; sc = NaN
        try
            img = load(f)
            gray = to_gray_u8(img)
    ...
    open(opts.outpath, "w") do io
        write(io, """
              ...
              <head>
              <meta charset="utf-8"/>
              <title>Photoscenary Detail Gallery</title>
              <style>
