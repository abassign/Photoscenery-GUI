...
using FileIO, ImageIO      # dipendenze dichiarate globalmente nel progetto
using Random
...
function _png_getter(path::AbstractString)
    img = FileIO.load(path)  # Colorant array o Array{UInt8,3}
...
# --- Util per DDS DXT1 (decodifica puntuale) ---
struct _DDSDXT1Ctx
    W::Int
    H::Int
    data::Vector{UInt8}  # dopo header (128B)
end
...
function _dds_dxt1_getter(path::AbstractString)
    io = open(path, "r")
    hdr = read(io, 128)
    (hdr[1]==0x44 && hdr[2]==0x44 && hdr[3]==0x53 && hdr[4]==0x20) || (close(io); error("DDS magic errato"))
    H = reinterpret(UInt32, hdr[13:16])[1] |> Int
    W = reinterpret(UInt32, hdr[17:20])[1] |> Int
    fourCC = String(Char.(hdr[85:88]))
    fourCC == "DXT1" || (close(io); error("DDS non DXT1 ($fourCC)"))
...
# --- Monte Carlo adattivo ---
"""
montecarlo_detail_score(H, W, getgray; max_samples=3000, min_samples=200,
batch=100, gedge=25, tol_rel=0.05, seed::Integer=0)

Ritorna (score, edge_density, lap_log, samples, H, W).
score = edge_density + 0.2 * log1p(var(Laplaciano)).
Stop precoce quando score si stabilizza o supera 1 con margine.
"""
...
function detail_score_file(path::AbstractString; kwargs...)
    ...
    if endswith(lp, ".png")
        ...
        error("Formato non supportato (attesi .png o .dds): $path")
    end
end
