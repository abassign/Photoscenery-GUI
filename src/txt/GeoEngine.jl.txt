...
export prepare_paths_and_location, process_target_area, create_precoverage_jobs, create_chunk_jobs, process_fill_holes, recover_orphaned_tiles, search_airports
...
# ---- Transitional shim (deprecabile più avanti) ----
...
Ricerca aeroporti per ICAO (match esatto a 4 lettere) o per nome/città (se disponibile nel modulo Route).
Ritorna un vettore di NamedTuple con campi: (icao, name, lat, lon, elev_ft).
"""
function __search_airports(q::AbstractString, limit::Integer=10)
    q_norm = strip(q)
    results = NamedTuple{(:icao, :name, :lat, :lon, :elev_ft)}[]

    # helper: estrai campo da NamedTuple o Dict (Symbol o String)
    ...
    # Caso 1: ICAO a 4 lettere -> risolvi direttamente
    ...
    # Caso 2: nome/città -> prova funzioni di ricerca se esistono in Route
    matches = Any[]
    ...
    elseif isdefined(Route, :find_airports)
        ...
        return results  # nessuna funzione di ricerca disponibile
    end
